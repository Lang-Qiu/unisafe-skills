...
Controlled copy of guard-llama-guard/scripts/calibrate.py (M2/M3 calibration
feature, same commit). Image adaptation: **none** — the logic is a pure
confidence-threshold sweep and is modality-agnostic. For shieldgemma2 the swept
`prediction.confidence` is the unsafe-direction max yes-probability; calibrate
on **CPU bf16 reference scores**, not int8 (int8 scores carry quantization
noise — calibrating on them calibrates to the artifact, see
references/shieldgemma2-notes.md section 6). Reuses this skill's metrics.py
loaders (themselves a controlled copy), so the eligibility/route semantics are
the image edition's.

Positive class = **unsafe**; FPR = FP/(FP+TN); answered records only; null
confidence excluded (identical population to the AUROC bucket). Guards without a
continuous score are reported as not-calibratable.
