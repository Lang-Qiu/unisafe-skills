...
scripts/validate.py. Image adaptation: none — coverage derives entirely from
this skill's utils.route_record (image eligible rules) and the canonical set
from this skill's category_mapping.json; the validation logic itself is
modality-agnostic. Any other difference from upstream is a defect. Note:
check_record only enforces required keys, so the AD-7 additive field
prediction.warnings passes untouched (locked by tests/test_validate.py).
