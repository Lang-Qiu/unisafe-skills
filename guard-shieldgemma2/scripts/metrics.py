...
Derived from guard-llama-guard@e5b2f70b3f0e8708495b051426d2d71677aa8afb
scripts/metrics.py. Image adaptation (the complete thin-diff list, plan AD-2;
anything else that differs from upstream is a defect):
  1. build_metrics: the text-truth buckets prompt_harm / pair_response_harm are
     emitted only when their rows are non-empty (image_safety records carry no
     prompt_is_unsafe/response_is_unsafe -> the buckets are naturally absent);
     head_binary always emitted; over_refusal_probe keeps upstream behavior
     (always emitted, n=0 on image data — honest absence of probe truth).
  2. compute_comparison: skips bucket names missing from any joined guard
     (consequence of 1; upstream indexed a fixed bucket list).
  3. --baseline default 'rule' -> 'caption-rule' (this skill's zero-dep baseline).
Eligibility comes from this skill's utils.route_record (image rules) — shared
import, not a diff.
...
Implements references/metrics-definitions.md exactly:
  ...
  - truth-field routing buckets: head / over_refusal_probe (text buckets absent
    on image data, see thin-diff 1)
  - v2 (metrics-definitions.md sections 9-12): --by-category (answered_only),
...
def compute_bucket(rows: List[Tuple[bool, Optional[bool], Optional[float]]]) -> Dict[str, Any]:
    ...
    if auroc_value is None:
        if not scored:
            auroc_reason = "no answered record has a non-null confidence (e.g. caption-rule baseline)"
        else:
...
def compute_adversarial_split(joined: List[Tuple[Dict[str, Any], Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    an honest data-gap surface, never silently merged into a slice. Image datasets
    typically lack the field entirely -> everything lands in unknown (M3_SPEC 4.3).
    """
...
def build_metrics(dataset: Dict[str, Dict[str, Any]],
    ...
    for guard_name, predictions in sorted(by_guard.items()):
        ...
        # thin-diff 1 (AD-2): text-truth buckets only when their rows exist
        buckets = {
            "head_binary": compute_bucket(rows("is_unsafe", lambda r: True)),
        }
        prompt_rows = rows("prompt_is_unsafe", lambda r: True)
        if prompt_rows:
            buckets["prompt_harm"] = compute_bucket(prompt_rows)
        pair_rows = rows("response_is_unsafe",
                         lambda r: r.get("task_type") == "prompt_response_safety")
        if pair_rows:
            buckets["pair_response_harm"] = compute_bucket(pair_rows)
...
def compute_comparison(results: Dict[str, Any], requested_baseline: str) -> Optional[Dict[str, Any]]:
    ...
    to the single-guard output shape. Buckets missing from any guard (text buckets
    on image data, thin-diff 2) or empty for every guard are omitted (no empty
    placeholders). Deltas are guard − baseline; the baseline is requested via
    --baseline (default caption-rule) and silently absent => note, no deltas.
    """
    if len(results) < 2:
        return None
    ...
    for bucket_name in ("head_binary", "prompt_harm", "pair_response_harm", "over_refusal_probe"):
        if any(bucket_name not in payload["buckets"] for payload in results.values()):
            continue  # thin-diff 2: conditionalized buckets may be absent
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--baseline", default="caption-rule",
                        help="baseline guard for comparison delta columns (default: caption-rule; "
