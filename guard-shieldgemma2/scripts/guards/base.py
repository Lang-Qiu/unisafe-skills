"""GuardAdapter protocol (M1_SPEC section 6; shared contract across guard skills).

Derived from guard-llama-guard@e72fa2bea4dde4a8252e2b6c3d76af8baa0e8f9f
scripts/guards/base.py. Image adaptation: none — the protocol is modality-
agnostic by design (the class-level `modality` default is overridden per
adapter); any other difference from upstream is a defect.

Adapters must be import-safe without their heavy dependencies installed:
defer torch/transformers imports to available()/predict().
"""
...
class GuardAdapter:
    ...
    Contract:
      ...
      - predict_batch(records): default = loop over predict; real batching
        adapters override and set capabilities.supports_batch.
    """
