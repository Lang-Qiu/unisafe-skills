...
5. Guard 输出的**文件布局**在 M0 契约中未定义（M0 §7 只定义了数据集侧），本 spec §5.4 给出定义，需作为 **M0 契约补充**开 PR 通知甲。（**状态 2026-06-11**：已写入 `M0_接口约定.md` §5，默认接受制生效中，否决窗口与其余甲侧事项见 `待甲确认.md`）
...
### 5.4 文件布局与 run_metadata（已写入 M0 §5；甲否决式确认中，见 `待甲确认.md` #1）
...
P0 契约与骨架（半天）
  ...
  T0.4 M0 契约补充（§5.4 布局）✅ 已落 M0 §5 + 待甲确认.md（待 push 通知甲） ┘
...
### 12.1 需要甲确认（契约/协作）——**已整理为 `待甲确认.md`，默认接受制（2026-06-11 推送）**
