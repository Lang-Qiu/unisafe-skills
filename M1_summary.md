...
| 真实 WildGuardTest 数据（任务 13） | 甲的 unified 输出未就绪；examples+fixtures 顶替（10 eligible），按 `待甲确认.md` #2 默认方案 | 提交前用真实数据重跑并重截图 |
...
- [x] E2E 截图（两 guard 指标矩阵运行画面）已存档 ✅（2026-06-11）——smoke 截图同已存档 ✅；**乙侧人工项至此清零**
- [ ] 甲：metrics-definitions.md 交叉 review（`待甲确认.md` #4，PR approve 即签字）
