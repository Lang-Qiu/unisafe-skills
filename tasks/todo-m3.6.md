...
- [~] 任务 1（E1a）生产精度 sweep：sg2 对 int8 全量、llama 对本身预测跑 `calibrate.py` → sweep 表落 `references/calibration-notes.md`；选点 **llama=max_macro_f1 0.55 / sg2(int8)=recall@FPR≤0.1 0.30**（**不照搬 CPU 0.10、不取 sg2 max_macro_f1=0.05/FPR38%**）。sg2 int8 sweep **plan 阶段已实跑**（OQ-2，`out_m3_real/calib_int8/`）；build 期补 llama sweep + 两侧写 calibration-notes（S · 依赖无）
- [ ] 任务 2（E1-llama 阈值机制，TDD · AD-1）：`llama_guard` 加 `is_unsafe=(confidence>=threshold)`，conf=None 回退 argmax，默认 0.55；测：阈值两侧判定 / 回退 / `threshold=0.5`≈argmax / `--threshold` 覆盖（M · 依赖 T1）
- [ ] 任务 3（E1-应用默认）：sg2 `DEFAULT_THRESHOLD`=**0.30** + llama 默认 **0.55** + config 回显；翻转用例显式 `threshold=0.5` + "默认=校准值"断言；黄金翻转则双路重锁；`--threshold 0.5` 复现旧行为；套件全绿（M · 依赖 T1,T2）
