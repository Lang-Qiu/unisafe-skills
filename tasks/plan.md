...
**Acceptance criteria**
- [ ] schema 可被 `json.load` 解析，`$schema`/`required`/枚举齐全
- [ ] 能表达三类非法：缺 `id`、`is_unsafe` 为字符串、孤儿 error——表达力由任务 8 的 test_validate 兑现
- [ ] 22 类枚举与 `references/category_mapping.json`（任务 3）同源一致

**Verification**
- [ ] `python -m json.tool schemas/guard_output.schema.json` exit 0
- [ ] 对 `root/M0_guard_output_sample.jsonl` 4 条逐条人工比对：每条都应合法
...
**Acceptance criteria**
- [ ] 与 spec §5 及 `root/M0_接口约定.md` §5 补充块零冲突（人工 diff）
- [ ] 错误三分法含触发条件、处理方式、exit code 影响三列；exit 表与全局 contract 一字不差
- [ ] 明确 eligible 定义：按路由规则 in-scope 的记录集合（任务 8/9 的覆盖率与指标分母依据）
...
**Acceptance criteria**
- [ ] mapping 三段齐全；S6 双映射、`other` 兜底显式
- [ ] metrics 文档含 5 项公式 + 双口径语义（answered-only=剔除 error；failure-as-wrong=error 计为错判的保守口径）+ 剔除规则 + over-refusal 定义 + 四计数字段公式（coverage=answered/eligible、error_rate=errors/eligible）

**Verification**
- [ ] `python -m json.tool` exit 0；与 M0 §4 表逐行 diff 一致
...
**Acceptance criteria**
- [ ] 6 条覆盖矩阵齐全；每行可 `json.loads`；id 保持 M0 原值
- [ ] Core-Minimal 链路（任务 5–10）声明的输入均指向本文件而非 root/M0_*

**Verification**
- [ ] `python -c "import json;[json.loads(l) for l in open('examples/input.sample.jsonl',encoding='utf-8')]"` exit 0
...
- [ ] 任务 1–4 产物存在且全部可解析
- [ ] `category_mapping.json` ↔ M0 §4 人工 diff 一致；schema 22 类枚举与之同源
- [ ] io-contract 内的 exit 表与本计划全局 contract 一致
- [ ] 甲侧事项已推送（`root/M1_待甲确认.md`，commit 7c7450e ✅）
...
**Acceptance criteria**
- [ ] `main.py --input examples/input.sample.jsonl --output-dir out_dry --dry-run` → exit 0；计数 total=6、eligible=5、skipped.out_of_scope=1
- [ ] run_metadata 含 spec §5.4 全部字段 + `eligible` 计数（guards 为空列表可）
- [ ] 输入路径不存在 / 0 条可解析 → exit 1 + 可读原因（contract 第 1 行语义）

**Verification**
- [ ] 上述命令 + exit code 三态实测；人工查看 run_metadata.json 字段
...
**Acceptance criteria**
- [ ] rule adapter 对 fixtures 的预测序列逐字段确定（test 断言锁定）
- [ ] 协议测试：默认 `predict_batch` 与逐条 `predict` 结果完全一致；`capabilities` 字段齐全
- [ ] 词表按 canonical 类别分组；探针误伤行为如实保留（spec §12.3-12）
- [ ] 不装任何第三方包时 `get_adapter('rule')` 可用

**Verification**
- [ ] `python -m unittest tests.test_rule_guard -v` 全绿；干净 venv import 冒烟
...
**Acceptance criteria**
- [ ] predictions 5 行；id 与输入逐字一致；`RESULT: ok predicted=5 errors=0 skipped=1`
- [ ] 重跑 `--resume` → 0 新增、exit 0；run_metadata 记录 `resume_hits / resume_misses / resume_hit_rate`（任务 17-E 的数据源）；exit 三态符合全局 contract
- [ ] 黄金文件与实跑字段级一致（latency/timestamp 除外），不含 error 行（error 示例归 fixtures）

**Verification**
- [ ] smoke 命令 + exit code 核对；test_rule_guard 黄金比对用例全绿
...
**Acceptance criteria**
- [ ] 黄金输出 → PASS exit 0；三类非法各 FAIL exit 1 且报错带行号
- [ ] `--against examples/input.sample.jsonl`：eligible 5/5 双向一致通过；人为删一行（缺失）→ FAIL；人为加幽灵 id（多余）→ FAIL
- [ ] 提供 `--metadata` 且 skip 计数不吻合（篡改 run_metadata）→ FAIL；**不提供 `--metadata` 时不校验计数**；image 记录无 prediction 不报错

**Verification**
- [ ] `python -m unittest tests.test_validate -v` 全绿；CLI 三态 exit 实测
...
**Acceptance criteria**
- [ ] 手算对照：answered-only Acc=0.75、Recall=0.75、FPR=0.25；failure-as-wrong Acc=0.60、Recall=0.60、FPR=0.40；coverage=0.8、error_rate=0.2 —— 全部精确断言
- [ ] Macro-F1 与 AUROC 手算值断言；rule（confidence=null）→ AUROC=null+原因标注
- [ ] 探针子集 FPR 有用例；未实现旗标响亮拒绝有用例；metrics.json 含四计数字段

**Verification**
- [ ] `python -m unittest tests.test_metrics -v` 全绿；对任务 7 真实输出跑 CLI，metrics.md 双口径表肉眼核对
...
**Acceptance criteria**
- [ ] ≤250 行；references 指针全存在；命令逐条复制实测可运行（bash + PowerShell）
- [ ] When NOT to use 含两个 dataset skill 与 checker 的互斥分工
- [ ] 三个 requirements 文件职责边界清晰；SKILL.md/README 的 exit 表与全局 contract 一字不差（含判例与 Plus 隔离原则）
- [ ] `pip install -r requirements.txt` exit 0 且 0 个第三方包被安装（`pip freeze` 前后一致）

**Verification**
- [ ] 行数统计；逐命令粘贴执行；exit 表 diff 核对
...
- [ ] 干净环境（无 torch/无网络/无 API key）全链 smoke——**固定 `--guards rule`，不跑 Plus/API guard**：main exit 0 → validate exit 0 → metrics 双口径产出，<60s
- [ ] `python -m unittest discover -s tests -v` 全绿（≥3 测试文件）
- [ ] **smoke 终端截图已存档**（第一张提交截图）
- [ ] 向用户汇报后进 P2/P3
...
**Acceptance criteria**
- [ ] `examples/input.sample.jsonl` 5 条 text ≥4 条非 error；confidence ∈ (0,1)；raw_output 保留 S 码
- [ ] spike 结论入 notes；SKILL.md `--timeout-s` 同步为"软超时"措辞
- [ ] 无 torch 环境 import 不崩；batch 路径与逐条路径结果一致（小样断言）

**Verification**
- [ ] `main.py --input examples/input.sample.jsonl --guards llama-guard --output-dir out_lg` → validate exit 0；人工抽 2 条对照原始输出
...
**Acceptance criteria**
- [ ] §9-J 第二项与 §9-N 三条全部实测打勾，exit code 符合全局 contract

**Verification**
- [ ] 按 §9-J/N 条目逐条执行并记录 exit code
...
**Acceptance criteria**
- [ ] metrics.md 矩阵含两 guard 全部适用指标（双口径）；AUROC 仅 llama-guard 有值且标注原因
- [ ] run_metadata 守恒式成立；E2E 截图存档；顶替情况如实记录

**Verification**
- [ ] `validate.py --against` 通过（eligible 口径）；抽 1 项指标手算复核
...
- [ ] §9-J 两项全勾；metrics.md 矩阵存在
- [ ] `references/metrics-definitions.md` 发甲启动交叉 review（待甲确认 #4）
...
**Acceptance criteria**
- [ ] 无 key：guard 级失败、exit 按判例（单独跑→1，与其他 guard 同跑→2）、提示清晰、其余 guard 不受影响
- [ ] 有 key：样例 5 条全部非 error；confidence 来自 category_scores，AUROC 可算
- [ ] **隔离原则落地**：不进入 C1 smoke 与默认 CI；无 key 的测试用例 `skipIf` 自动跳过（`unittest discover` 不依赖 key）；N/A 仅记录于显式 Plus 命令场景，不影响 C1/C3

**Verification**
- [ ] 两种环境各跑一次 main.py；validate exit 0
...
**Acceptance criteria**
- [ ] 样例数据（含 1 条探针）跑出 over_refusal_rate 且带 low_sample_warning
- [ ] 模板占位符与 metrics.md 字段一一对应

**Verification**
- [ ] `python -m unittest tests.test_metrics -v`（追加用例）全绿
...
**Acceptance criteria**
- [ ] eval 表完整（prompt、预期、实测/判定、结论）；不达标时给 description 修订建议（回流任务 18）
...
**Acceptance criteria**
- [ ] ablations.md 含 A–G 七节，每节结论或 N/A+原因，**不删节**
- [ ] 至少 E/F/G 三个零依赖子项有实测数据
- [ ] 可直接引用进报告"分析"章节的至少 1 张表/曲线数据

**Verification**
- [ ] 文档自检：每节有可复现命令记录；抽 1 子项重跑核对数字
...
**Acceptance criteria**
- [ ] §9-B 全勾；与 trigger-eval 结论一致；未达成项的 sanity 行标注"待 Core-Full（顺延 M2）"而非删除

**Verification**
- [ ] 行数统计 + 命令重测 + 指针遍历 + exit 表 diff
...
**Acceptance criteria**
- [ ] 追溯表五行齐全且文件/测试名真实；N/A 记录符合全局"不删项"规则；C2 失败场景下本文件完整呈现降级事实
- [ ] 含 **Extension Backlog 表**：列 = Extension 项 / 入口接口 / 当前状态 / 顺延阶段；至少覆盖 Scope Note 所列项（LLM-as-judge、WildGuard 适配器、多模态 image safety、vLLM/高吞吐后端、完整性能消融），入口接口列须指向真实存在的扩展点（guards 注册表、`predict_batch`、metrics 分桶旗标、schema 的 modality 字段等）
...
**Acceptance criteria**
- [ ] §9 Core-Minimal 必需项全勾；其余项勾或 N/A；截图归档；甲 #4 已发出

**Verification**
- [ ] 验收清单即核对表；git push 成功
...
- [ ] §9 清单中 **Core-Minimal 必需项全勾**；Core-Full/Plus 项勾或 N/A（含原因与顺延里程碑）
- [ ] `root/M1_summary.md` 追溯表完整
